...
| file | what it is |
|---|---|
| `tgu.service` | `tgu export chats` then `tgu ship`, `Type=oneshot`, aborts on the first failure |
| `tgu.timer` | `OnCalendar=*-*-* 04:17:00`, `Persistent=true` |

`install.sh` is **deferred**, not forgotten. It is the only non-trivial artefact
in TASK-14 — create the `tgu` user, `install -d -m 0700 -o tgu /srv/tgu`, clone,
...
```sh
sudo useradd --system --create-home --shell /usr/sbin/nologin tgu
sudo install -d -m 0700 -o tgu -g tgu /srv/tgu
sudo install -m 0644 deploy/tgu.service deploy/tgu.timer /etc/systemd/system/
sudo systemctl daemon-reload && sudo systemctl enable --now tgu.timer
```

## /etc/tgu.env (root:tgu 0640)
...
1. `tgu export chats` holds a full Telegram account credential and never calls
   gbrain or an LLM.
2. `tgu ship` talks to gbrain and holds no Telegram credential. It is a
